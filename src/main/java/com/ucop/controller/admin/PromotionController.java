...
public class PromotionController {
    ...
    private void handleCreate() {
        try {
            PromotionDAO dao = createDAOFromForm();
            promotionService.createPromotion(dao);
    ...
    private void handleUpdate() {
        ...
        try {
            PromotionDAO dao = createDAOFromForm();
            promotionService.updatePromotion(selectedPromotion.getId(), dao);
    ...
    private PromotionDAO createDAOFromForm() {
        PromotionDAO dao = new PromotionDAO();
        dao.setCode(codeField.getText());
        dao.setName(nameField.getText());
        dao.setDescription(descriptionArea.getText());
        dao.setDiscountType(discountTypeCombo.getValue());
        dao.setDiscountValue(new BigDecimal(discountValueField.getText()));
        
        if (!minOrderField.getText().isEmpty()) {
            dao.setMinOrderAmount(new BigDecimal(minOrderField.getText()));
        }
        if (!maxDiscountField.getText().isEmpty()) {
            dao.setMaxDiscountAmount(new BigDecimal(maxDiscountField.getText()));
        }
        if (!maxUsageTotalField.getText().isEmpty()) {
            dao.setMaxUsageTotal(Integer.parseInt(maxUsageTotalField.getText()));
        }
        
        dao.setStartDate(startDatePicker.getValue().atStartOfDay());
        dao.setEndDate(endDatePicker.getValue().atTime(23, 59, 59));
        dao.setMaxUsagePerUser(Integer.parseInt(maxUsagePerUserField.getText()));
        dao.setActive(activeCheckBox.isSelected());
        dao.setStackable(stackableCheckBox.isSelected());
