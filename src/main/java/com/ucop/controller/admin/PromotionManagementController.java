...
public class PromotionManagementController {
    ...
    public void setDashboardController(DashboardController dashboardController) {
        System.out.println("✓ PromotionManagementController.setDashboardController() called with: " + 
            (dashboardController == null ? "NULL" : dashboardController.getClass().getSimpleName()));
        this.dashboardController = dashboardController;
    }
    
    public void setAdminDashboardController(AdminDashboardController adminDashboardController) {
        System.out.println("✓ PromotionManagementController.setAdminDashboardController() called");
    ...
    private void handleCreate() {
        try {
            System.out.println("=== handleCreate() called ===");
            System.out.println("dashboardController: " + (dashboardController == null ? "NULL" : "NOT NULL"));
            System.out.println("adminDashboardController: " + (adminDashboardController == null ? "NULL" : "NOT NULL"));
            
            if (dashboardController != null) {
                System.out.println("✓ Calling dashboardController.showPromotionForm()");
                dashboardController.showPromotionForm(null);
            } else if (adminDashboardController != null) {
                System.out.println("✓ Calling adminDashboardController.showPromotionForm()");
                adminDashboardController.showPromotionForm(null);
            } else {
                System.out.println("✗ Both controllers are NULL! Opening fallback dialog");
    ...
    private void handleEdit(Promotion promotion) {
        try {
            if (dashboardController != null) {
                ...
                adminDashboardController.showPromotionForm(promotion);
            } else {
                // Fallback: open in new window if not in dashboard
    ...
    private void openPromotionFormDialog(Promotion promotion) throws Exception {
        // Load form vào màn hình chính thay vì mở cửa sổ mới
        System.out.println("DEBUG: dashboardController is " + (dashboardController == null ? "NULL" : "NOT NULL"));
        if (dashboardController != null) {
            System.out.println("✓ Loading form in main screen...");
            dashboardController.showPromotionForm(promotion);
            return; // QUAN TRỌNG: Return ngay để không chạy code bên dưới
        } else {
            System.out.println("✗ Dashboard controller is null, opening dialog...");
            // Fallback: Nếu không có dashboard controller, vẫn mở dialog như cũ
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/UI/admin/promotion-form.fxml"));
