...
public class CartRepositoryImpl implements CartRepository {
    ...
    public Optional<Cart> findById(Long id) {
        try (Session session = sessionFactory.openSession()) {
            Cart cart = session.createQuery(
                ...
                .setParameter("id", id)
                .setCacheable(false) // Disable cache to always get fresh data
                .uniqueResult();
    ...
    public Optional<Cart> findByAccountId(Long accountId) {
        try (Session session = sessionFactory.openSession()) {
            ...
            query.setCacheable(false); // Disable cache to always get fresh data
    ...
    public void update(Cart cart) {
        ...
        try (Session session = sessionFactory.openSession()) {
            transaction = session.beginTransaction();
            session.merge(cart);
            session.flush(); // Force update to DB
            transaction.commit();
        } catch (Exception e) {
            if (transaction != null) transaction.rollback();
            throw e;
